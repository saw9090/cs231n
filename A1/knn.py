...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    x_train_re=x_train.reshape(num_train,-1)
    x_test_re=x_test.reshape(num_test,-1)
    for j in range(num_test):
        for i in range(num_train):
            dists[i,j]=torch.sqrt(torch.sum((x_train_re[i]-x_test_re[j])**2))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_re=x_test.reshape(num_test,-1)
    for i in range(num_train):
        dists[i]=torch.sqrt(torch.sum((x_test_re-x_train_re[i])**2,axis=1))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_re=x_train.reshape(num_train,-1)
    x_test_re=x_test.reshape(num_test,-1)
    sum=torch.matmul(x_train_re,x_test_re.t())*(-2)+torch.sum(x_test_re**2,axis=1)+torch.sum(x_train_re**2,axis=1).view(-1,1)
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        _,idx=torch.topk(dists[:,i],k,largest=False)
        y_pred[i], _=torch.mode(y_train[idx])
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices :
        ...
        for i in range(num_folds):
            ...
            x_train_list=list(x_train_folds)
            y_train_list=list(y_train_folds)
            x_train_list.pop(i)
            y_train_list.pop(i)
            x_train = torch.cat(x_train_list,0)
            y_train = torch.cat(y_train_list,0)
            classifier = KnnClassifier(x_train, y_train)
            list0.append(classifier.check_accuracy(x_val, y_val, k=k))
        k_to_accuracies[k]=list0
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
