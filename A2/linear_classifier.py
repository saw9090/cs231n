...
def svm_loss_vectorized(
    ...
    C = W.shape[1]
    score = torch.mm(X,W)
    collect_class_score = score[range(N),y].reshape(N,1)
    margin = score-collect_class_score+1
    margin[margin<0] = 0
    margin[range(N),y] = 0
    loss = margin.sum()/N
    loss += reg * torch.sum(W * W)
    ...
    #############################################################################
    margin[margin>0] = 1
    margin_sum= margin.sum(axis=1)
    margin[range(N),y] = (-1) * margin_sum
    dW = torch.mm(X.T,margin)/N
    dW += reg * 2 * W
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    ###########################################################################
    y_pred = torch.argmax(torch.mm(X,W), dim=1)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return y_pred
...
def svm_get_search_params():
    ...
    regularization_strengths = [1,5e-1]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    for i in range(num_iters) : 
        ...
        train_acc = (cls.predict(data_dict['X_train']) == data_dict['y_train']).float().mean().item()
        val_acc = (cls.predict(data_dict['X_val']) == data_dict['y_val']).float().mean().item()
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train) : 
        ...
        score_soft = torch.exp(score)/torch.sum(torch.exp(score))
        loss -= torch.log(score_soft[y[i]])
        for j in range(num_class) : 
            dW[:,j] += (score_soft[j]-(1 if j==y[i] else 0))*X[i]
    loss /= num_train
    dW /= num_train
    loss += reg * torch.sum(W * W)
    dW += reg * 2 * W
...
def softmax_loss_vectorized(
    ...
    #loss
    score = torch.mm(X,W)
    score_max = torch.max(score,1)[0].reshape(N,1)
    score -= score_max
    score = torch.exp(score)
    score_soft = score/(torch.sum(score,1).reshape(N,1))
    loss = (-1) * torch.sum(torch.log(score_soft[range(N),y]))

    #dW
    score_soft[range(N),y] -= 1
    dW = torch.mm(X.T,score_soft)

    #scaling and regularzation
    loss /= N
    dW /= N
    loss += reg * torch.sum(W * W)
    dW += reg * 2 * W
...
def softmax_get_search_params():
    ...
    regularization_strengths = [2e-2, 1e-2]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
