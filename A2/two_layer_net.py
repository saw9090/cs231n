...
def nn_forward_backward(
    ...
    ############################################################################
    N = X.shape[0]
    scores_max = torch.reshape(torch.max(scores,1)[0],(N,1))
    scores -= scores_max
    scores = torch.exp(scores)
    scores_soft = scores/(torch.sum(scores,1).reshape(N,1))
    loss = (-1) * torch.sum(torch.log(scores_soft[range(N),y]))
    loss /= N
    ...
    grads['b2'] = torch.sum(dscores,0)
    grads['W2'] = torch.mm(h1.T,dscores)

    dh1 = torch.mm(dscores,W2.T)
    dh1 = dh1 * (h1 > 0)

    grads['b1'] = torch.sum(dh1,0)
...
def nn_get_search_params():
    ...
    # classifier.                                                             #
    ###########################################################################
    learning_rates = [1, 1.2]
    hidden_sizes = [ 2048, 1024]
    regularization_strengths = [1e-3, 5e-4]
